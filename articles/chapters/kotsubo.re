...
@<code>{generate_topics} は「リサーチアプリケーションの実装」の節で紹介した関数です。@<code>{generate_trivia} はそれと並行して実行する新しい関数で、クエリに関連する豆知識を1件返します。
 ...
 * @<b>{追加のレイテンシがほぼゼロ}：豆知識生成はトピック生成と並行して走るため、この機能を追加しても全体の待ち時間はほとんど増えません
